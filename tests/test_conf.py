...
from dwim import const


@pytest.fixture
def load_source(mocker):
    return mocker.patch('dwim.conf.load_source')


@pytest.fixture
def environ(monkeypatch):
    data = {}
    monkeypatch.setattr('dwim.conf.os.environ', data)
    return data
...
class TestSettingsFromEnv(object):
    def test_from_env(self, environ, settings):
        environ.update({'DWIM_RULES': 'bash:lisp',
                        'DWIM_EXCLUDE_RULES': 'git:vim',
                        'DWIM_WAIT_COMMAND': '55',
                        'DWIM_REQUIRE_CONFIRMATION': 'true',
                        'DWIM_NO_COLORS': 'false',
                        'DWIM_PRIORITY': 'bash=10:lisp=wrong:vim=15',
                        'DWIM_WAIT_SLOW_COMMAND': '999',
                        'DWIM_SLOW_COMMANDS': 'lein:react-native:./gradlew'})
        settings.init()
    ...
    def test_from_env_with_DEFAULT(self, environ, settings):
        environ.update({'DWIM_RULES': 'DEFAULT_RULES:bash:lisp'})
        settings.init()
...
@pytest.mark.parametrize('legacy_dir_exists, xdg_config_home, result', [
    (False, '~/.config', '~/.config/dwim'),
    (False, '/user/test/config/', '/user/test/config/dwim'),
    (True, '~/.config', '~/.dwim'),
    (True, '/user/test/config/', '~/.dwim')])
def test_get_user_dir_path(mocker, environ, settings, legacy_dir_exists,
                           xdg_config_home, result):
    mocker.patch('dwim.conf.Path.is_dir',
