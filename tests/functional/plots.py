def _set_confirmation(proc, require):
    proc.sendline(u'mkdir -p ~/.dwim')
    proc.sendline(
        u'echo "require_confirmation = {}" > ~/.dwim/settings.py'.format(
            require))
...
def history_not_changed(proc, TIMEOUT):
    ...
    proc.send('\033[A')
    assert proc.expect([TIMEOUT, u'dwim'])
