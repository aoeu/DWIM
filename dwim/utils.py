...
def cache(*depends_on):
    ...
    def _get_cache_path():
        ...
        cache_path = Path(cache_dir).joinpath('dwim').as_posix()
...
def get_installation_info():
    return pkg_resources.require('dwim')[0]


def get_alias():
    return os.environ.get('TF_ALIAS', 'dwim')


@memoize
def get_valid_history_without_current(command):
    def _not_corrected(history, tf_alias):
        """Returns all lines from history except that comes before `dwim`."""
        previous = None
        for line in history:
