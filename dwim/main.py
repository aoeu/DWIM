...
def fix_command():
    """Fixes previous command. Used when `dwim` called without arguments."""
    settings.init()
...
def how_to_configure_alias():
    ...
    It'll be only visible when user type dwim and when alias isn't configured.
...
def main():
    parser = ArgumentParser(prog='dwim')
    version = get_installation_info().version
    parser.add_argument(
            '-v', '--version',
            action='version',
            version='DWIM {} using Python {}'.format(
